...
mkEqnHelp overlap_mode tvs cls cls_tys tycon tc_args mtheta
  = do {      -- Find the instance of a data family
       ...
       ; let (rep_tc, rep_tc_args, rep_co) = tcLookupDataFamInst fam_envs tycon tc_args
             rep_kind_co = mkSymCo (mkKindCo rep_co)
       ...
       ; if isDataTyCon rep_tc then
            mkDataTypeEqn dflags overlap_mode tvs cls cls_tys
                          tycon tc_args rep_tc rep_tc_args rep_kind_co mtheta
         else
...
mkDataTypeEqn dflags overlap_mode tvs cls cls_tys
  ...
  where
    go_for_it    = mk_data_eqn overlap_mode tvs cls tycon tc_args rep_tc rep_tc_args rep_kind_co mtheta
    bale_out msg = failWithTc (derivingThingErr False cls cls_tys (mkTyConApp tycon tc_args) msg)
...
mk_data_eqn :: Maybe OverlapMode -> [TyCoVar] -> Class
            ...
            -> DerivContext
            -> TcM EarlyDerivSpec
mk_data_eqn overlap_mode tvs cls tycon tc_args rep_tc rep_tc_args _rep_kind_co mtheta
  = do loc                  <- getSrcSpanM
...
mkNewTypeEqn :: DynFlags -> Maybe OverlapMode -> [TyCoVar] -> Class
             -> [Type] -> TyCon -> [Type] -> TyCon -> [Type] -> Coercion
             -> DerivContext
             -> TcRn EarlyDerivSpec
mkNewTypeEqn dflags overlap_mode tvs
             cls cls_tys tycon tc_args rep_tycon rep_tc_args rep_kind_co mtheta
...
genInst comauxs
        spec@(DS { ds_tvs = tvs, ds_tc = rep_tycon
